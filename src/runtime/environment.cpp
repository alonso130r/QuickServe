...
struct ModelDeleter {
  void operator()(llama_model *p) const { llama_model_free(p); }
};
struct ContextDeleter {
  void operator()(llama_context *p) const { llama_free(p); }
};
struct SamplerDeleter {
  void operator()(llama_sampler *p) const { llama_sampler_free(p); }
};
struct BatchGuard {
...
double effective_bytes_per_scalar(ggml_type type) {
  return static_cast<double>(ggml_type_size(type)) /
         static_cast<double>(ggml_blck_size(type));
}
...
std::optional<llama_token> choose_synthetic_token(const llama_vocab *vocab) {
  ...
  for (std::int32_t token = 0; token < count; ++token) {
    if (llama_vocab_is_eog(vocab, token))
      continue;
    std::string piece;
    if (to_piece(vocab, token, piece) && !piece.empty())
      return token;
  }
  return std::nullopt;
}
...
void Environment::run() {
  ...
  try {
    ...
    const std::optional<ModelProfile> model_profile = make_model_profile({
        llama_model_n_params(model.get()),
        llama_model_size(model.get()),
        llama_model_n_layer(model.get()),
        llama_model_n_embd(model.get()),
        llama_model_n_head(model.get()),
        llama_model_n_head_kv(model.get()),
        effective_bytes_per_scalar(params.type_k),
        effective_bytes_per_scalar(params.type_v),
        config_.context_size,
        config_.batch_capacity,
        config_.max_sequences,
    });
    if (!model_profile) {
      publish_startup({false, "invalid model metadata"});
      return;
    }
    ...
    if (!startup_batch.token || !startup_batch.pos || !startup_batch.n_seq_id ||
        !startup_batch.seq_id || !startup_batch.logits) {
    ...
    publish_startup({true, {}, model_profile});
  ...
  while (!handoff_.stop_requested()) {
    try {
      ...
      while (handoff_.try_take_release(release)) {
        ...
        retry(ack, [this](const ReleaseAck &m) {
          return handoff_.try_acknowledge_release(m);
        });
      }
      ...
      for (;;) {
        if (!pending_admission) {
          Admission admission{};
          if (!handoff_.try_take_admission(admission))
            break;
          progressed = true;
        ...
        if (fatal) {
          ...
          try {
            if (requests.count(pending.admission.id) != 0) {
              ...
              pending.prompt =
                  tokenize(vocab, pending.admission.prompt, true, true);
        ...
        if (result.error == ErrorCode::None && free_sequences.empty())
          break;
        ...
        if (plan) {
          ...
          for (const WorkItem &work : plan->work) {
            ...
            if (found == requests.end() || !seen.insert(work.id).second ||
                ...
                static_cast<std::uint64_t>(batch.n_tokens) +
                        work.token_count() >
                    config_.batch_capacity) {
              valid = false;
              break;
            ...
            if ((prefill && (work.token_begin != record.prefill_position ||
                ...
                (!prefill &&
                 (work.kind != WorkKind::Decode || record.decoded_tokens == 0 ||
                  work.token_begin !=
                      record.prompt.size() + record.decoded_tokens - 1 ||
                  work.token_end != work.token_begin + 1))) {
              valid = false;
              break;
            }
            for (std::uint32_t pos = work.token_begin; pos < work.token_end;
                 ++pos) {
              const std::int32_t row = batch.n_tokens++;
              batch.token[row] =
                  prefill ? record.prompt[pos] : record.last_token;
            ...
            const bool output =
                !prefill || work.token_end == record.prompt.size();
          ...
          for (Pending &item : pending) {
            ...
            completion.prefill_position =
                item.work.kind == WorkKind::Prefill
                    ? item.work.token_end
            ...
            if (item.logits_row >= 0) {
              const llama_token token = llama_sampler_sample(
                  item.record->sampler.get(), context.get(), item.logits_row);
              if (token < 0) {
                fail_run(ErrorCode::SamplingFailed);
                break;
              }
              ...
              if (!completion.eos) {
                std::string piece;
                if (!to_piece(vocab, token, piece)) {
                  fail_run(ErrorCode::SamplingFailed);
                  break;
                }
                OutputPiece output{item.work.id, token, std::move(piece)};
                retry(output, [this](OutputPiece &m) {
                  return handoff_.try_report_output(std::move(m));
                });
              }
            }
            retry(completion, [this](const Completion &m) {
              return handoff_.try_report_completion(m);
            });
          }
        }
      }
