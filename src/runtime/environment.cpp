...
std::vector<llama_token> tokenize(const llama_vocab *vocab, const std::string &text) {
  ...
  const std::int32_t required = llama_tokenize(vocab, text.data(), size, nullptr, 0, true, true);
  if (required == INT32_MIN || required >= 0) return {};
  std::vector<llama_token> tokens(static_cast<std::size_t>(-required));
  const std::int32_t count = llama_tokenize(vocab, text.data(), size, tokens.data(), static_cast<std::int32_t>(tokens.size()), true, true);
  if (count <= 0) return {};
...
void Environment::run() {
  ...
  while (!handoff_.stop_requested()) {
    ...
    Admission admission{};
    while (handoff_.try_take_admission(admission)) {
      progressed = true;
      AdmissionResult result{admission.id, 0, ErrorCode::None};
      try {
      if (fatal) result.error = ErrorCode::EnvironmentStopped;
      else if (requests.count(admission.id) != 0) result.error = ErrorCode::ProtocolViolation;
      else {
        auto tokens = tokenize(vocab, admission.prompt);
        if (tokens.empty()) result.error = ErrorCode::TokenizationFailed;
        else if (tokens.size() + static_cast<std::uint64_t>(admission.max_output_tokens) > config_.context_size || free_sequences.empty())
          result.error = ErrorCode::ContextCapacityExceeded;
        else {
          SamplerPtr sampler(llama_sampler_init_greedy());
          if (!sampler) result.error = ErrorCode::SamplingFailed;
          else {
            RequestRecord record{};
            record.prompt = std::move(tokens);
            record.sequence = free_sequences.back();
            free_sequences.pop_back();
            record.sampler = std::move(sampler);
            result.prompt_tokens = static_cast<std::uint32_t>(record.prompt.size());
            requests.emplace(admission.id, std::move(record));
          }
        }
      }
      } catch (...) {
      ...
      retry(result, [this](const AdmissionResult &m) { return handoff_.try_report_admission(m); });
    }
